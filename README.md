...
Plan 3 已完成——時間軸（Day 分頁、色塊拖曳含自動連鎖順延／🔒 鎖定不被連鎖波及／5 分鐘吸附）、播放頭與地圖「我」標記（每秒推進 10 分鐘）、停留點時間全面改用當地時區顯示。

Plan 4 已完成——相鄰停留點交通段自動計算（Google Routes，含 route_cache 快取）、交通段編輯器（手動修正時長／交通方式，flight 段跨日跨時區起訖，手動內容不被自動計算覆蓋）、交通與停留銜接衝突警示。地圖路線圖層（選中日 polyline 實線 + flight 大圓弧虛線）已隨播放視覺（2026-08-04）補齊。

播放視覺已完成（2026-08-04）——分段取景（依交通段距離自動換縮放）、交通工具圖示播放頭（✈️ SVG 沿弧旋轉）、紅線沿真實道路漸進繪製；分享頁同步生效（get_shared_trip 白名單補回 polyline，migration 20260804000000）。
...
- ✅ Plan 4 完成：交通段自動計算（Google Routes）+ 手動修正 + flight 跨日跨時區起訖 + 交通與停留衝突警示
- ✅ 播放視覺完成：選中日路線圖層 + 分段取景 + 交通工具圖示 + 漸進紅線（分享頁同步生效）
...
- 斷線（Realtime 連線中斷）時顯示橫幅並暫停側欄新增停留點的寫入入口（地圖右鍵、搜尋加入、拖曳移動時間）；已開啟的停留點/交通段編輯器與刪除鈕不受影響，重連後自動清除橫幅並整份重新整理
