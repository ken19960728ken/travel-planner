...
- **手繪交通路徑**：Google 算不出來（例如日本的電車）或算得不對時，可在地圖上自己點出轉折點描出實際走向——拖曳頂點調整、拖線段中央插入新點、手機也能畫。畫出來的路徑會取代自動路線出現在地圖、播放動畫、分享頁與匯出檔中；隨時可「還原成自動路線」。這是你自己的資料，不受 Google 30 天快取限制，會永久保存進定稿快照
...
測試現況：單元／整合 306 項、Playwright E2E 8 條（含分享 token 外洩迴歸、Realtime 雙 context 同步、匿名攻擊防護），皆綠。
