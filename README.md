...
- 邀請連結（`trip_invites`）多次可用直到過期（預設 7 天、上限 30 天）或 owner 手動撤銷；owner 移除某成員後，若該成員手上仍有未過期的邀請連結，可直接重新加入——移除操作目前不會連帶失效既有邀請，屬已知設計缺口，留待 Task 7 成員面板一併處理（db-expert/critic 審查共識）
