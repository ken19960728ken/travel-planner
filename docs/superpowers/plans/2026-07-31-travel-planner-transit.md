...
- [ ] **Step 3: 驗證** — lint / tsc / build 綠；手動（dev server + 本地 Supabase + 兩顆同行程停留點）：`curl -X POST http://localhost:3000/api/trips/<id>/legs/sync -H 'Cookie: <登入 cookie>'` 或直接由 Task 5 的 UI 接線後驗證（推薦後者，cookie 手工太折騰——本 Task 先保證編譯與單元層，端到端行為由 Task 5 手動驗 + Task 8 E2E 防守，如實回報選了哪條路）。未設 `GOOGLE_MAPS_SERVER_API_KEY` 時端點回 `{ ok: true, pending: N }` 不報錯。→ **Commit** `feat: 交通段同步端點（Routes 代理、route_cache、限流、ToS TTL）`
